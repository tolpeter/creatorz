...
/**
 * "Expanding cards" carousel a kiemelt tartalomgyártókhoz (asztali + mobil).
 * A layout tisztán CSS (globals.css `.fc-*`), a rotáció/aktiválás imperatív
 * DOM-logika (a listaelemeket ténylegesen átmozgatjuk), a Simey-féle demo
 * nyomán — a projektbe adaptálva. A React csak egyszer rendereli a listát,
 * utána nem re-renderel (nincs state), így nincs ütközés a DOM-mozgatással.
 */
...
export function FeaturedExpandingCarousel({
  ...
  useEffect(() => {
    ...
    const indexOf = (el: Element | null) => (el ? items().indexOf(el as HTMLElement) : -1);
    ...
    const onChoose = (e: Event) => {
      // Névre kattintás → hagyjuk navigálni a profilra.

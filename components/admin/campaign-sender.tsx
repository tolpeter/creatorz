...
import { sendProfilePhotoBatch, sendProfilePhotoTest } from "@/app/actions/campaigns";
...
export function CampaignSender({
  ...
  function runTest() {
    setBusy("test");
    start(async () => {
      const res = await sendProfilePhotoTest();
      setBusy(null);
  ...
  function runBatch() {
    setBusy("batch");
    start(async () => {
      const res = await sendProfilePhotoBatch(BATCH);
      setBusy(null);
      if (res.error) toast.error(res.error);
      else {
        toast.success(
  ...
  return (
    <div className="rounded-2xl border bg-card p-5 shadow-sm">
      <div className="mb-3 flex items-center gap-2">
        ...
        <div>
          <p className="font-bold">Profilkép-ösztönző kampány</p>
          <p className="text-xs text-muted-foreground">
            Tartalomgyártók profilkép nélkül. A küldés a szerveren fut (éles Resend-kulcs).
          </p>
        </div>
      </div>
      ...
      <div className="mb-4 grid grid-cols-3 gap-3">
        <div className="rounded-xl border bg-background p-3 text-center">
          ...
          <p className="text-[11px] text-muted-foreground">profilkép nélkül</p>
        </div>
        <div className="rounded-xl border bg-background p-3 text-center">
      ...
      <p className="mt-2 text-[11px] text-muted-foreground">
        Tipp: előbb a „Teszt magamnak" gombbal nézd meg a levelet, utána indítsd a kiküldést.
      </p>
    </div>
  );
}
